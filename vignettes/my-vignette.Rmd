...
Next, let's consider our statistical summaries function, which provides us wiht a table. Here, we can observe statistical summaries of the body weights of the mice over the three checkpoints as well as compare the bodyweight groups against each other. It calculates summary statistics for each checkpoint, performs a paired t-test to compare body weights between the first and third checkpoints, and conducts an ANOVA to compare body weights across all checkpoints.
```{r}
stat_summary()
```
